...
export async function GET(request, { params }) {
  try {
    // ✅ Correct way to get params in Next.js App Router (params is a Promise)
    ...
    if (!API_KEY || !SITE_ID) {
      return NextResponse.json({ error: "Wix API credentials missing" }, { status: 500 });
    }
    ...
    // Optional: Fetch rich content from draft-posts if not included
    ...
    const post = {
      ...
      excerpt: p.customExcerpt || p.excerpt || (p.contentText ? p.contentText.slice(0, 200).trim() + "..." : ""),
      ...
      url: p.url?.path || null,
      seoTitle: p.seoData?.tags?.find(t => t.type === "title")?.children || p.title,
      seoDescription: p.seoData?.tags?.find(t => 
        t.type === "meta" && t.props?.name === "description"
      )?.props?.content || p.excerpt || "",
      metrics: p.metrics || {},
    };
